...
import pdb
import argparse
from prompts.prompt_template_persona2 import persona_com_instruct_generate_rewrite_wo_persona
...
def parse_args():
    ...
    parser.add_argument(
        ...
        type=str,
        # required=True,
        default="/home/dyf/data_generate/doc-instruct/data/lima/persona2/persona_add_lima_persona2.jsonl",
...
def random_sample(seed_tasks, roundi, is_vllm, batch_length, model, sampling_params, chat_formatting_function, documents, tokenizer, model_id): #随机选择数据进行generate
    all_logs = []
    ...
    if is_vllm == True:
        ...
        for idx in tqdm(range(len(seed_tasks))): # range(len(seed_tasks))
            ...
            if True:# quality_score_vllm(question, model, sampling_params, chat_formatting_function):
                ...
                # t['conversations'].append({"from":"user", "value": })
                all_logs.append(t)
                output_log_jsonl(os.path.join('/home/dyf/data_generate/doc-instruct/data/lima/epoch/com/', f"com_new_instruct_round_{roundi}.jsonl"), all_logs) 
                # output log at each iteration
                # merge_log = seed_tasks + all_logs
                if roundi == 1:
...
def random_sample_record(seed_tasks, batch_length, roundi): #选择过的数据会被记录选择次数
    ...
    for idx in range(len(task)): #len(seed_tasks)
        ...
        if filter_output(documents, question) and quality_score(question):
            ...
            all_logs.append(t)
            # output log at each iteration
            output_log_jsonl(os.path.join("/home/dyf/data_generate/doc-instruct/data/lima/epoch", f"com_new_instruct_{batch_length}_round_{roundi}.jsonl"), all_logs) 
...
def random_sample_record_UCB(seed_tasks, batch_length, roundi, th): #记录次数，并加入UCB评分进行采样
    ...
    for idx in range(len(task)): #len(seed_tasks)
        ...
        if filter_output(documents, question) and quality_score(question):
            ...
            all_logs.append(t)
            # output log at each iteration
            output_log_jsonl(os.path.join("/home/dyf/data_generate/doc-instruct/data/lima/epoch", f"com_new_instruct_{batch_length}_round_{roundi}_wo_vllm.jsonl"), all_logs) 
...
# def main_com(roundi, seed_tasks, is_vllm, batch_length, model, sampling_params, chat_formatting_function, documents, tokenizer):
#     # args = parse_args()
...
#     # os.makedirs(args.batch_dir, exist_ok=True)
    # return random_sample(seed_tasks, roundi, is_vllm, batch_length, model, sampling_params, chat_formatting_function, documents, tokenizer)
